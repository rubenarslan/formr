...
`r finished(results)` completed rows, `r started` who entered any information, `r only_viewed` only viewed the first page. There are `r expired(results)` expired rows (people who did not finish filling out in the requested time frame). In total, there are `r nrow(results)` rows including unfinished and expired rows. There were `r users` unique participants, of which `r finished_users` finished filling out at least one survey.
...
```{r}
pander::pander(missingness_patterns(results[!is.na(results$ended), ] ))
```
...
```{r}
pander::pander(missingness_patterns(results[!is.na(results$modified), ] ))
```
