...
```{r}
ggplot2::qplot(results$created) + scale_x_datetime("Date/time when survey was started")
```

People took on average `r mean(duration$duration)` minutes (median `r median(duration$duration)`) to answer the survey.
...
for (i in seq_along(vars)) {
	...
	if ( !is.null(item_info)) {
		if (exists("part_of_scale", item_info) || exists("scale", item_info) || var %in% c("created", "modified", "expired")) {
			next
...
## Items
```{r}
DT::datatable(dplyr::select(as.data.frame(items(results)), -choice_list, -id, -study_id, -index), rownames = FALSE, filter = "top", extensions = 'Buttons', options = list(
    dom = 'Bfrtip',
