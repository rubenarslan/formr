...
R package `formr` (CRAN), the client-side companion to the rforms.org survey framework. The same package is *also* loaded server-side inside rforms.org via openCPU, so exported functions are called both from local user R sessions and from within running formr studies — avoid changes that silently break either context.
...
- **OAuth client-credentials path** (`R/formr_api.R`) — `formr_api_access_token()` hits `https://api.rforms.org/oauth/access_token`, stores the URL + token in the `.formr_current_session` closure, and `formr_api_results()` / `formr_api_session()` read from there. This is a separate, much narrower surface.
...
- `formr_last_host()` is authoritative for the active host across cookie-path functions; new functions that talk to rforms.org should default `host = formr_last_host()` rather than hardcoding a URL.
